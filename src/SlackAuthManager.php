...
/**
 * Contains all the logic for Slack login integration.
 */
class SlackAuthManager extends OAuth2Manager {
   ...
   * The Slack user.
   *
   * @var \League\OAuth2\Client\Provider\SlackUser
   */
  protected $user;
  ...
  public function __construct(LoggerChannelFactoryInterface $logger_factory, EventDispatcherInterface $event_dispatcher, EntityFieldManagerInterface $entity_field_manager, UrlGeneratorInterface $url_generator, ConfigFactory $configFactory) {
    ...
    $this->config             = $configFactory->getEditable('social_auth_slack.settings');
  }
  ...
  public function getSlackLoginUrl() {
    ...
    $slack_scopes = explode(PHP_EOL, $this->getScopes());
    foreach ($slack_scopes as $scope) {
